...
for(species_now in species_list){
    adatas[[species_now]]$var[[paste0(species_now, "_homolog_associated_gene_name")]] = adatas[[species_now]]$var_names
}
...
## version = '105'
mart <- useEnsembl("ensembl", version = "106", dataset = as.character(paste(species_1, "_gene_ensembl", sep = "")))
...
write_csv(homology_tbl, path = homology_tbl_csv)
...
distinct(get(paste0(species_1, "_homolog_associated_gene_name")), `.keep_all` = TRUE)
...
for(species_now in names(adatas)){
    adatas_one2one[[species_now]] = adatas[[species_now]][, tolower(adatas[[species_now]]$var_names) %in% tolower(one2one[[paste0(species_now, "_homolog_associated_gene_name")]])]
}

for(species_now in species_list[-1]){
    message(species_now)
     one2one_now = one2one %>% filter(tolower(get(paste0(species_now, "_homolog_associated_gene_name"))) %in% tolower(adatas_one2one[[species_now]]$var_names)) %>%
distinct(get(paste0(species_1, "_homolog_associated_gene_name")), `.keep_all` = TRUE)
     adatas_one2one[[species_now]] = adatas_one2one[[species_now]][, tolower(adatas_one2one[[species_now]]$var_names) %in% tolower(one2one_now[[paste0(species_now, "_homolog_associated_gene_name")]])]
     one2one_now = one2one_now[match(tolower(adatas_one2one[[species_now]]$var_names), tolower(one2one_now[[paste0(species_now, '_homolog_associated_gene_name')]])), ]
     adatas_one2one[[species_now]]$var[[paste0(species_now, '_homolog_associated_gene_name')]] = adatas_one2one[[species_now]]$var_names
     adatas_one2one[[species_now]]$var[[paste0(species_1, '_homolog_associated_gene_name')]] = one2one_now[[paste0(species_1, '_homolog_associated_gene_name')]]
     adatas_one2one[[species_now]]$var_names = one2one_now[[paste0(species_1, '_homolog_associated_gene_name')]]
...
for(species_now in species_list){
    many2many = many2many %>% filter(!is.na(get(paste0(species_now, "_homolog_associated_gene_name"))) & get(paste0(species_now, "_homolog_associated_gene_name")) != "") %>%
    filter(get(paste0(species_now, "_homolog_associated_gene_name")) %in% adatas[[species_now]]$var[[paste0(species_now, "_homolog_associated_gene_name")]])
}
...
while (nrow(many2many_copy) > 0) {
    ...
    dd <- many2many_copy %>%
        filter(get(paste0(species_1, "_homolog_associated_gene_name"))  == levels(factor(many2many_copy[[paste0(species_1, "_homolog_associated_gene_name")]]))[1])
    genes_now = dd %>% dplyr::select(ends_with("homolog_associated_gene_name")) %>% flatten() %>% unique() %>% as.character()
    gene_group <- many2many_copy %>%
        dplyr::filter_at(vars(ends_with("homolog_associated_gene_name")), any_vars(. %in% genes_now))
    ...
    for(species_now in species_list){
            adatas_many2many[[species_now]] = adatas[[species_now]][, adatas[[species_now]]$var_names %in% gene_group[[paste0(species_now, "_homolog_associated_gene_name")]]]
            keep_row = adatas_many2many[[species_now]]$var %>%
            arrange(desc(mean_counts)) %>%
            slice(1)
            adatas_many2many[[species_now]] = adatas_many2many[[species_now]][, which(adatas_many2many[[species_now]]$var_names == rownames(keep_row))]
        }

    new_name = adatas_many2many[[species_1]]$var_names
    for(species_now in species_list[-1]){
    adatas_many2many[[species_now]]$var[[paste0(species_1, "_homolog_associated_gene_name")]] = new_name
    rownames(adatas_many2many[[species_now]]$var) = new_name
}
    adata_add = concat(adatas_many2many, axis = 0L, join = 'inner', merge = 'first')
...
## mind the order
avail_homo = c(avail_attributes$name[grepl("orthology_confidence", avail_attributes$name)],
              avail_attributes$name[grepl("homolog_goc_score", avail_attributes$name)],
              avail_attributes$name[grepl("homolog_wga_coverage", avail_attributes$name)])

while (nrow(many2many_copy_homo) > 0) {

    dd <- many2many_copy_homo %>%
        filter(get(paste0(species_1, "_homolog_associated_gene_name"))  == levels(factor(many2many_copy_homo[[paste0(species_1, "_homolog_associated_gene_name")]]))[1])
    ...
    adatas_many2many_homo[[species_now]] = adatas[[species_now]][, adatas[[species_now]]$var_names %in% gene_keep[[paste0(species_now, "_homolog_associated_gene_name")]]]
    ...
    adatas_many2many_homo[[species_now]]$var[[paste0(species_1, "_homolog_associated_gene_name")]] = new_name
    rownames(adatas_many2many_homo[[species_now]]$var) = new_name

}
    adata_add = concat(adatas_many2many_homo, axis = 0L, join = 'inner', merge = 'first')
